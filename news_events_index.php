<?php
	...
	//echo 'del-id';
		...
		$stmt_select = $DB_con->prepare('SELECT image_path FROM news_items WHERE userID =:uid');
		...
		$stmt_delete = $DB_con->prepare('DELETE FROM news_items WHERE userID =:uid');
		$stmt_delete->bindParam(':uid',$_GET['delete_id']);
		$stmt_delete->execute();
		//echo $_GET['delete_id'];
		header("Location: news_events_index.php");
	}
            ...
            <div class="row">
                <div class="col-sm-12">
                    <ol class="breadcrumb">
                        ...
                        <li class="active"><h1><span class="highlight">EDIT / DELETE NEWS & EVENTS DETAILS</span></h1></li>
                    </ol>
                </div>
...
<?php
	...
	$stmt = $DB_con->prepare('SELECT userID, news_header, subheading, image_path,  description FROM news_items ORDER BY userID DESC');
	$stmt->execute();
	...
	if($stmt->rowCount() > 0)
	{
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			...
			?>
			<div class="col-xs-3">
				<p class="page-header"><?php echo $news_header."&nbsp;/&nbsp;".$subheading.$description; ?></p>
				<img src="user_images/<?php echo $row['image_path']; ?>" class="img-rounded" width="150px" height="150px" />
				<p class="page-header">
				<span>
				<a class="btn btn-info" href="news_events_editform.php?edit_id=<?php echo $row['userID']; ?>" title="click for edit" onclick="return confirm('Are you sure to edit ?')"><span class="glyphicon glyphicon-edit"></span> Edit</a> 
